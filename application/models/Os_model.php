...
class Os_model extends CI_Model
{
    ...
    public function getOs($table, $fields, $where = [], $perpage = 0, $start = 0, $one = false, $array = 'array')
    {
        ...
        $this->db->select($fields . ',clientes.idClientes, clientes.nomeCliente, clientes.celular as celular_cliente, usuarios.nome, veiculos.marca, garantias.*');
        $this->db->from($table);
        $this->db->join('clientes', 'clientes.idClientes = os.clientes_id');
        $this->db->join('usuarios', 'usuarios.idUsuarios = os.usuarios_id');
		$this->db->join('veiculos', 'veiculos.idVeiculos = os.veiculos_id');
    ...
    {
        $this->db->select('os.*, clientes.*, clientes.celular as celular_cliente, garantias.refGarantia, usuarios.telefone as telefone_usuario, usuarios.email as email_usuario, usuarios.nome, veiculos.marca');
        $this->db->from('os');
        $this->db->join('clientes', 'clientes.idClientes = os.clientes_id');
        $this->db->join('usuarios', 'usuarios.idUsuarios = os.usuarios_id');
		$this->db->join('veiculos', 'veiculos.idVeiculos = os.veiculos_id');
	  ...
	  public function autoCompleteVeiculo($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = ['label' => $row['marca'] . ' | modelo: ' . $row['modelo'] . ' | placa: ' . $row['placa'], 'id' => $row['idVeiculos']];
            }
            echo json_encode($row_set);
        }
    }
