...
class Clientes_model extends CI_Model
{
    ...
    public function get($table, $fields, $where = '', $perpage = 0, $start = 0, $one = false, $array = 'array')
    {
        ...
        $this->db->from($table);
		//$this->db->join('veiculos', 'veiculos.idVeiculos = '.$table.'.veiculos_id');
    ...
    public function getOsByCliente($id)
    {
	   ...
	   public function getVeiculosByCliente($id)
    {
        $this->db->where('clientes_id', $id);
		...
		public function autoCompleteVeiculo($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = ['label'=>$row['marca'].' | Modelo: '.$row['modelo'],'id'=>$row['idVeiculos']];
            }
            echo json_encode($row_set);
        } else {
            $row_set[] = ['label'=> 'Adicionar veiculo...', 'id' => null];
            echo json_encode($row_set);
        }
    }
	*/
