...
<?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aVeiculo')) { ?>
    <a href="<?php echo base_url(); ?>index.php/veiculos/adicionar" class="button btn btn-mini btn-success" style="max-width: 160px">
      <span class="button__icon"><i class='bx bx-plus-circle'></i></span><span class="button__text2">Add. Veiculos</span></a>
	<!--  
        ...
        <table id="tabela" class="table table-bordered ">
            ...
            foreach ($results as $r) {
                ...
                if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eVeiculo')) {
                    echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/veiculos/editar/' . $r->idVeiculos . '" class="btn-nwe3" title="Editar Veiculo"><i class="bx bx-edit bx-xs"></i></a>';
                }
                if ($this->permission->checkPermission($this->session->userdata('permissao'), 'dVeiculo')) {
                    echo '<a style="margin-right: 1%" href="#modal-excluir" role="button" data-toggle="modal" veiculo="' . $r->idVeiculos . '" class="btn-nwe4" title="Excluir Veiculo"><i class="bx bx-trash-alt bx-xs"></i></a>';
                }
                echo '</td>';                                        
                echo '</tr>';
            } ?>
...
<div id="modal-excluir" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form action="<?php echo base_url() ?>index.php/veiculos/excluir" method="post">
        <div class="modal-header">
            ...
            <h5 id="myModalLabel"><i class="fas fa-trash-alt"></i> Excluir Veiculo</h5>
        </div>
        <div class="modal-body">
            <input type="hidden" id="idVeiculo" class="idVeiculo" name="id" value=""/>
...
<div id="atualizar-estoque" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form action="<?php echo base_url() ?>index.php/veiculos/atualizar_estoque" method="post" id="formEstoque">
        <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
        ...
        <div class="modal-body">
            ...
            <div class="control-group">
                <label for="estoque" class="control-label">Adicionar Veiculos<span class="required">*</span></label>
                <div class="controls">
                    <input type="hidden" id="idVeiculo" class="idVeiculo" name="id" value=""/>
...
<div id="modal-etiquetas" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form action="<?php echo base_url() ?>index.php/relatorios/veiculosEtiquetas" method="get">
	   ...
	   <div class="modal-header">
        ...
        <div class="modal-body">
            <div class="span12 alert alert-info" style="margin-left: 0"> Escolha o intervalo de veiculos para gerar as etiquetas.</div>

            <div class="span12" style="margin-left: 0;">
                <div class="span6" style="margin-left: 0;">
                    <label for="valor">De</label>
                    <input class="span9" style="margin-left: 0" type="text" id="de_id" name="de_id" placeholder="ID do primeiro veiculo" value=""/>
                </div>


                <div class="span6">
                    <label for="valor">Até</label>
                    <input class="span9" type="text" id="ate_id" name="ate_id" placeholder="ID do último veiculo" value=""/>
                </div>
